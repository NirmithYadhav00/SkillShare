...
const GLOBAL_CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  .login-input{width:100%;padding:13px 16px;border-radius:12px;border:1.5px solid ${C.border};background:#fff;color:${C.text};font-family:'Inter',sans-serif;font-size:.95rem;outline:none;transition:all .2s;}
  .login-input::placeholder{color:${C.dim};}
  .login-input:focus{border-color:${C.bAccent};box-shadow:0 0 0 3px rgba(0,86,210,.08);}
  .login-btn{width:100%;padding:13px;border-radius:12px;border:none;cursor:pointer;font-family:'Inter',sans-serif;font-weight:700;font-size:1rem;color:#fff;background:${C.a1};box-shadow:0 6px 20px rgba(0,86,210,.25);transition:all .2s;}
  .login-btn:hover{transform:translateY(-2px);background:${C.a2};box-shadow:0 10px 28px rgba(0,86,210,.35);}
...
function Login() {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch("https://skillshare-ebe1.onrender.comapi/auth/login", {
        method: "POST",
      ...
      <div style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "center", minHeight: "calc(100vh - 68px)", padding: "60px 5%", overflow: "hidden" }}>
        ...
        <div style={{
          ...
          <div style={{ textAlign: "center", marginBottom: 32 }}>
            ...
            <p style={{ fontSize: "0.9rem", color: C.muted, fontWeight: 400 }}>Sign in to your Skill Share account</p>
          </div>
          ...
          <p style={{ textAlign: "center", marginTop: 24, fontSize: "0.88rem", color: C.muted, fontWeight: 500 }}>
            Don't have an account? <Link to="/Signup" style={{ color: C.a1, fontWeight: 700, textDecoration: "none" }}>Sign Up</Link>
          </p>
